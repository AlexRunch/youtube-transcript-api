...
# Настройка локального времени (MSK)
...
@app.route('/')
def home():
    return "Welcome to the YouTube Transcript API!", 200

# Пинг для проверки работы сервиса
@app.route('/ping')
def ping():
...
def get_transcript():
    ...
    try:
        ...
        # Поиск субтитров в заданном окне времени
...
# Запуск сервера (Heroku будет использовать gunicorn)
